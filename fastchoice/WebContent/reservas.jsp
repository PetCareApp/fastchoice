...
<body>
    ...
    <div class="row">
        ...
        <div class="span12 gallery">
			...
			<div class="row">
			<div class="span16">
            <form action="SearchReserva" method="post" id="custom-search-form" class="form-search form-horizontal pull-right">
	...
	<%
	    List<Estabelecimento> estabelecimentosReservados = (List<Estabelecimento>) session.getAttribute("estabelecimentosReservados");
		...
		if (estabelecimentosReservados == null || estabelecimentosReservados.isEmpty()) {
		    estabelecimentosReservados = new ArrayList<Estabelecimento>();
		    for (Reserva r: minhasReservas) {
				Estabelecimento estab = (Estabelecimento) Dao.getInstance().readWithId("estabelecimento", r.getIdEstabelecimento());
				estabelecimentosReservados.add(estab);
		    }
		}
	    if (estabelecimentosReservados != null && estabelecimentosReservados.size() > 0) {
			Collections.sort(estabelecimentosReservados, new Comparator() {
			    @Override
			    public int compare(Object e1, Object e2) {
				return ((Estabelecimento) e1).getNome().compareTo(((Estabelecimento) e2).getNome());
			    }
			});
			for (Estabelecimento e : estabelecimentosReservados) {
			    String idEstab = e.getId() + "";
			    String data = "";
			    for (Reserva r : minhasReservas) {
					if (r.getIdEstabelecimento() == e.getId()) {
					    data += r.getData();
					}
			    }
	%>
...
<li  class="span3 gallery-item" data-id="id-1" data-type="illustration">
	<form role="form" action="SelectSearchEstabelecimento" method="post" class="SelectSearchEstabelecimento">
		...
		<span class="gallery-icons">
			...
			<a href="gallery-single.htm"><%=e.getNome()%></a><%=data%><br><%=e.getDescricao()%>
			</span>
			</form>
		 </li>
			<%}
